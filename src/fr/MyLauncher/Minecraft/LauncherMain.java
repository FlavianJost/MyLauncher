...
public class LauncherMain extends AlternativeBase{
	...
	private GameFolder gamefolder = new GameFolder("RetrocraftPvP");File f=new File(gamefolder.binDir,"auth_infos.json");
	private LauncherPreferences launcherpreferences = new LauncherPreferences("RetrocraftPvP Launcher v1.0", 950, 600, Mover.MOVE);
	private GameLinks gameLinks = new GameLinks("http://retrocraftpvp.synology.me/Launcher/", "1.10.2.json");
	...
	public GameConnect gameConnect = new GameConnect("revolium.minesr.com", "25565");
	...
	private Parent createContent() {
		...
		boolean login=f.exists();//System.out.println(login);
	
		new LauncherAccount(contentPane, gameEngine, gamefolder, f);
		
		/*if(login==false) {
			new LauncherLogin(contentPane,gameEngine,gamefolder,f);
		}else if(login==true) {
			new LauncherPanel(contentPane, gameEngine,gamefolder);
		}
		//System.out.println(config.getValue(EnumConfig.CONNECT));
		//System.out.println(EnumConfig.CONNECT);
		//System.out.println(config.getValue(EnumConfig.AUTOLOGIN));
		/*if(config.getValue(EnumConfig.CONNECT)=="1") {
			new LauncherPanel(contentPane, this.GAME_ENGINE);
		}else if(config.getValue(EnumConfig.CONNECT)=="0") {
			new LauncherAccount(contentPane, this.GAME_ENGINE);
		}*/
